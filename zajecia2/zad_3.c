...
typedef struct {
    ...
    Point p3;
    Point p4;
} Rectangle; //struktura Rectangle skladajaca sie z struktur Point

//metoda zmienia wspolrzedne punktow
void changeCoordinates(Point * p) {
    p->x = 5;
    p->y = 5;
}

//metoda obliczajaca odleglosc miedzy punktami
float distanceCalculator(Point p1, Point p2) {
    return (float)sqrt(pow((p2.x - p1.x), 2) + pow((p2.y - p1.y),2));
}

//metoda oblicza pole i obwod prostokata a nastepnie je wypisuje
