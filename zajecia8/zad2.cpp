...
//Part I
int sumValidGames(string str) {
    int num, gameID = 0;
    regex reg("(\\d+)\\s+(\\w+)");
    regex reg2("Game\\s+(\\d+)"); 
    smatch match;
  
    if (regex_search(str, match, reg2)) {
        istringstream is(match.str(1));
        is >> gameID;
    }
    
    while (regex_search(str, match, reg)) {
        istringstream iss(match.str(0));
        iss >> num;
        if (match.str(0).find("green") != string::npos && num > 13) {
            return 0;
        }
        else if (match.str(0).find("red") != string::npos && num > 12) {
            return 0;
        }
        else if (match.str(0).find("blue") != string::npos && num > 14) {
            return 0;
        }
        str = match.suffix();
    }
...
int dicePowerCount(string str) {
    ...
    while (regex_search(str, match, reg)) {
        istringstream iss(match.str(0));
        iss >> num;
        if (match.str(0).find("green") != string::npos && greenVal < num) {
            greenVal = num;
        }
        else if (match.str(0).find("red") != string::npos && redVal < num) {
            redVal = num;
        }
        else if (match.str(0).find("blue") != string::npos && blueVal < num) {
            blueVal = num;
        }
        str = match.suffix();
    }
